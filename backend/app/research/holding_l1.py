...
def base_model_contract(model):
    return json.loads(json.dumps(dict(spec=asdict(model.spec), symbols=list(model.symbols),
        calibrated=hasattr(model, 'calibration'), feature_version='direction_holding_v1')))
...
class HoldingL1Residual:
    def fit(self,book,base_oos,targets,label_end,*,before,base_trained_before,base_contract=None):
...
def packet_books(packet,as_of):
    ...
    if packet.get('mode', 'live')!='live':raise ValueError('Diagnostic replay packets cannot enter live decisions')
...
def apply_residuals(models,books,as_of,symbols,expected_base_contract=None):
    adjustment=dict.fromkeys(symbols,0.);status={}
    for s in symbols:
        try:
            if expected_base_contract is not None and models[s].model.artifact.get('base_model_contract')!=expected_base_contract:
                raise ValueError('L1 residual trained against a different base forecast model')
