# backend/data/run_weekly_market_replay.py
"""
Weekly Market Replay Benchmark:
Runs Quant.ai Two-Stage Hierarchical ML Engine across all 5 trading days of the past week (2026-08-25 to 2026-09-01).
Uses Real Account $676k Buying Power (4x Margin Leverage) and Instant Target Position Flipping (w_t*).
"""
...
def replay_entire_week():
    ...
    print("================================================================================")
    print("🗓️ QUANT.AI PAST 5 TRADING DAYS (LAST WEEK) MARKET REPLAY BENCHMARK")
    print("================================================================================\n")
    print(f"💰 Starting Account Net Equity: ${real_equity:,.2f} USD")
    print(f"🚀 Intraday 4x Buying Power:     ${real_buying_power:,.2f} USD")
    print(f"🎯 Target Focus Watchlist:       {', '.join(symbols)}\n")

    # Fetch 1 month of 5m data for all symbols
    stock_dfs = {}
    for s in symbols:
        try:
            ...
            print(f"⚠️ Error downloading {s}: {e}")

    # Extract unique trading dates from the available data
    if not stock_dfs:
    ...
    print(f"📅 Selected 5 Trading Days: {', '.join(last_5_dates)}\n")
            ...
            for i in range(5, len(df_session)):
                ...
                if target_w != w_t:
                    ...
                    if w_t != 0.0:
                        ...
                        if pnl_pct <= -0.012: # 1.2% Risk Stop Loss
                            trade_pnl = entry_shares * (close - entry_price) if w_t > 0 else entry_shares * (entry_price - close)
                            day_pnl += trade_pnl
                            day_trades += 1
                            if trade_pnl > 0:
    ...
    print("\n================================================================================")
    print("🏆 PAST 5 TRADING DAYS (LAST WEEK) CUMULATIVE REPLAY SUMMARY")
    print("================================================================================")
