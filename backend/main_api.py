...
def archive_history_endpoint(keep_days: int = 2):
    ...
    res = live_runner.sync_to_huggingface()
    return res


@app.post("/api/live/sync_hf")
def sync_hf_endpoint():
    """Uploads master trade_history.json and daily partitions to HuggingFace Dataset repository (Ypeng12/quant-ai-trade-history)."""
    return live_runner.sync_to_huggingface()
...
async def get_wave_day_data(ticker: str = "TSLA", date: str = ""):
    """
    On-demand lazy-loading endpoint for wave data.
    Supports both real-time today live calculation and historical day cached recall.
    """
    ...
    is_today_request = (dt.lower() in ["today", "live", "latest", "now"]) or (dt == today_ny)

    # If requested for today or live, dynamically compute from real-time market data
    if is_today_request:
        try:
            from app.ml.lob_wave_realtime import compute_live_wave_day_data
...
def get_ml_prediction_trajectory(ticker: str = "SNDK", date: Optional[str] = None):
    ...
    try:
        ...
        return {
            ...
            "opens": [round(float(x), 2) for x in today_df['Open'].values],
            "highs": [round(float(x), 2) for x in highs_arr],
            "lows": [round(float(x), 2) for x in lows_arr],
            "volumes": [int(x) for x in volumes_arr],
