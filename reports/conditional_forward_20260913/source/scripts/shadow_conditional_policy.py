#!/usr/bin/env python3
"""Frozen paper-inspired conditional model decisions; no broker imports or order submission."""
import argparse
...
def register_policy(bundle,directory,candidate='conditional_state_h1'):
    result=load_platform_results(bundle)
    if result['status']!='complete':raise ValueError(result.get('reason'))
    research=result['research']
    c=next(c for c in LIQUID_CANDIDATES if c.name==candidate)
    ...
    return register(directory,dict(candidate=c.name,input_symbols=research['symbols'],cost_bps=5,
        ...
        list(dict.fromkeys([*research['sources'],'scripts/shadow_conditional_policy.py','backend/app/research/shadow.py','backend/app/research/conditional_policy.py'])))
...
def record(directory,bars,instruments,account_snapshot,*,now=None):
    ...
    c=next(c for c in LIQUID_CANDIDATES if c.name==cfg['candidate'])
    engine=LiquidRuntime
    ...
    payload=dict(available_at=available.isoformat(),candidate=c.name,current_weights=current,target_weights=weights,
        eligibility=context['eligibility'],training=context['training'],selection=context['selection'],
...
def main():
    ...
    q.add_argument('--candidate',default='conditional_state_h1',choices=[c.name for c in LIQUID_CANDIDATES if not(c.control or c.benchmark)])
