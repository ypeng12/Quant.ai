#!/usr/bin/env python3
"""Paper-inspired conditional momentum against fixed Ridge and equal allocation."""
...
from app.research.conditional_policy import ConditionalRuntime as LiquidRuntime,CONDITIONAL_CANDIDATES as LIQUID_CANDIDATES
...
def run(output):
    ...
    sources=[Path(__file__).resolve(),*[ROOT/'backend/app'/p for p in ('research/conditional_policy.py','research/stock_policy.py','research/liquid_policy.py','research/strategy_extensions.py','research/platform.py','research/data_audit.py','research/policy_replay.py','research/causal_week_replay.py','research/attribution.py','research/artifacts.py','quant_policy.py')]]
    r=dict(schema_version=1,status='running',registered_at=pd.Timestamp.now(tz='UTC').isoformat(),evaluation_dates=dates,symbols=list(files),starting_equity=100000,
        ...
        selection_rule=None,
        literature=dict(title='Market Intraday Momentum',authors='Gao, Han, Li, Zhou',url='https://ssrn.com/abstract=2440866',transfer='Hypothesis adaptation, not exact replication: stock returns throughout session at 5/30 minutes instead of ETF first/last half-hour'),
        hypothesis='Observed volume and volatility interact with momentum; coefficients learned from prior sessions',
    ...
    for c in LIQUID_CANDIDATES:
        ...
        if c.name=='conditional_ridge_control' and abs(summary['net_pnl']-1169.2454058230069)>1e-6:raise AssertionError('Fixed Ridge control mismatch')
