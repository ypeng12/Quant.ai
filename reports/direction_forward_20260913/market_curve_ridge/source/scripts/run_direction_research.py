#!/usr/bin/env python3
"""Four-stock direction-model changes and fully costed holding-plan comparison."""
...
from app.research.direction_policy import DirectionRuntime as LiquidRuntime,DIRECTION_CANDIDATES as LIQUID_CANDIDATES,REFERENCES
...
def run(output):
    ...
    sources=[Path(__file__).resolve(),*[ROOT/'backend/app'/p for p in ('research/direction_policy.py','research/liquid_policy.py','research/strategy_extensions.py','research/platform.py','research/data_audit.py','research/policy_replay.py','research/causal_week_replay.py','research/attribution.py','research/artifacts.py','quant_policy.py')]]
    r=dict(schema_version=1,status='running',registered_at=pd.Timestamp.now(tz='UTC').isoformat(),evaluation_dates=dates,symbols=list(files),starting_equity=100000,
        ...
        calibration='Uncalibrated regression forecasts; no probability or guaranteed direction claims',
        integrated_risk='Risk aligned with the forecast horizon; sequential cross-stock covariance for the six-step holding plan',
        risk_preferences=dict(risk_aversion=50,gross_limit=.95,symbol_limit=.7,planned_terminal_liquidation=True,execution="Only first planned step executes; future steps are forecasts, not actual fills"),
