...
def planned_target(forecasts,covariance,current,symbols,eligible,spec):
    ...
    bounds=Bounds(np.r_[-limits,np.zeros(2*m)],np.r_[limits,np.full(2*m,np.inf)])
    ...
    initial=np.clip(old,-limits[:n],limits[:n])
    ...
    planned=np.clip(x[:m],-limits,limits).reshape(k,n);planned[abs(planned)<1e-10]=0
    for w in planned:
