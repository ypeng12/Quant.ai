...
class ResearchRuntime:
    ...
    def prepare(self,c,day,index):
        ...
        if risk_key not in self.risks:
            cov=horizon_covariance(self.frames,day,symbols,c.horizon)
            if c.reference_risk:cov=cov+reference_covariance(self.frames,day,symbols,c.horizon)
        ...
        if model_key not in self.forecasts:
            ...
            if c.model=='pair':
                ...
                for s in symbols:
                    lk=(s,c.horizon)
                    if lk not in self.labels:self.labels[lk]=labeled_frame(self.frames[s],ResearchSpec('labels',horizon_bars=c.horizon))[1]
                    ys[s]=self.labels[lk]
