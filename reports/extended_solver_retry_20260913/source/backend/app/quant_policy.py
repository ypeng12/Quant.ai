...
def panel_feature_frames(raw_by_symbol: Mapping[str, pd.DataFrame], feature_set: str = "price_volume") -> dict[str, pd.DataFrame]:
    ...
    # Reindex each symbol's own gap-validated returns; a missing bar in every
    # peer must not silently turn a five-minute return into ten minutes.
    returns_1 = pd.concat({symbol: base[symbol].return_1 for symbol in symbols}, axis=1)
    returns_3 = pd.concat({symbol: base[symbol].return_3 for symbol in symbols}, axis=1)
    frames = {}
...
def target_weights(mu: Mapping[str, float], covariance, current_weights: Mapping[str, float],
                   ...
                   symbols=None, *, uncertainty=None, uncertainty_aversion: float = 0.0,
                   costs_bps=None, solver: str = "slsqp") -> dict[str, float]:
    ...
    cost = np.full(n, spec.cost_bps / 10000.0) if costs_bps is None else np.asarray(costs_bps, dtype=float) / 10000.0
    error = np.zeros(n) if uncertainty is None else np.asarray(uncertainty, dtype=float)
    if (cost.shape != (n,) or error.shape != (n,) or not np.isfinite(cost).all()
            or not np.isfinite(error).all() or (cost < 0).any() or (error < 0).any()
            or not np.isfinite(uncertainty_aversion) or uncertainty_aversion < 0):
        raise ValueError("Aligned finite nonnegative cost and forecast uncertainty required")
    uncertainty_penalty = error * uncertainty_aversion
    objective_scale = max(np.abs(expected).max(), spec.risk_aversion * np.abs(cov).max(), cost.max(), uncertainty_penalty.max(),
                          np.finfo(float).eps)

    def objective(x):
        w = x[:n]
        return (-expected @ w + 0.5 * spec.risk_aversion * w @ cov @ w + cost @ x[n:2*n] + uncertainty_penalty @ x[2*n:]) / objective_scale

    def gradient(x):
        return np.concatenate((-expected + spec.risk_aversion * cov @ x[:n],
                               cost, uncertainty_penalty)) / objective_scale
    ...
    if solver == "osqp":
        ...
        quadratic = sparse.block_diag((spec.risk_aversion * cov / objective_scale,
                                       sparse.csc_matrix((2*n, 2*n))), format="csc")
        linear = np.concatenate((-expected, cost, uncertainty_penalty)) / objective_scale
        all_constraints = sparse.vstack((sparse.csc_matrix(matrix), sparse.eye(3*n)), format="csc")
        low = np.concatenate((lower, bounds.lb)); high = np.concatenate((np.full(len(lower), np.inf), bounds.ub))
        problem = osqp.OSQP()
        problem.setup(P=sparse.triu(quadratic, format="csc"), q=linear, A=all_constraints, l=low, u=high,
                      verbose=False, eps_abs=1e-8, eps_rel=1e-8, max_iter=100000, polishing=True)
        problem.warm_start(x=x0)
        solution = problem.solve(raise_error=False)
        if solution.info.status != "solved" or not np.isfinite(solution.x).all():
            raise RuntimeError(f"Portfolio QP failed: {solution.info.status}")
        violation = max(float(np.max(low - all_constraints @ solution.x)),
                        float(np.max(all_constraints @ solution.x - high)))
        if violation > 1e-6:
            raise RuntimeError("Portfolio QP solution violates constraints")
        solved = solution.x
    elif solver == "slsqp":
        ...
        raise ValueError("Unknown portfolio solver")
    weights = np.clip(solved[:n], lower_w, spec.symbol_limit)
