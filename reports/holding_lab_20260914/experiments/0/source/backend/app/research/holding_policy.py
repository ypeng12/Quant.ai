...
class HoldingModel:
    ...
    def allocation(self, forecasts, current, stamp, allowed=None, shortable=None, l1_adjustment=None):
        symbols = self.symbols; allowed = set(symbols if allowed is None else allowed)
        stamp = pd.Timestamp(stamp)
        remaining = int((stamp.normalize() + pd.Timedelta(hours=15, minutes=55) -
        ...
        explanation.update(model=self.spec.name, feature_family=self.spec.family,
            ...
            cumulative_forecast_bps={s:{str(h*5):float(forecasts[s][h])*10000 for h in self.spec.horizons} for s in symbols},
    ...
    def live_target(self, frames, current, *, allowed, shortable, as_of):
        ...
        return self.allocation(values,current,stamp,allowed,shortable)
    ...
    def load(cls,path):
        ...
        if a.get('kind')!=VERSION: raise ValueError('Unsupported holding model')
