...
def run(args):
    ...
    registration=dict(status='registered',created_at=pd.Timestamp.now(tz='UTC').isoformat(),
        ...
        sources={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),ROOT/'backend/app/research/holding_policy.py',ROOT/'backend/app/research/direction_policy.py',ROOT/'backend/app/alpha/paper_bar_alpha.py',ROOT/'backend/app/research/paper_models.py']})
    ...
    for spec in CANDIDATES:
        ...
        for day in dates:
            model=HoldingModel.fit(frames,pd.Timestamp(day,tz='America/New_York'),spec,features=x)
        ...
        deploy=HoldingModel.fit(frames,pd.Timestamp(args.end,tz='America/New_York')+pd.Timedelta(days=1),spec,features=x)
...
if __name__=='__main__':
    ...
    p.add_argument('--append-today',action='store_true');run(p.parse_args())
