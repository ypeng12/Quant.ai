#!/usr/bin/env python3
"""Registered incremental Alpha hypotheses; frozen conditional Ridge control."""
...
from app.research.incremental_policy import IncrementalRuntime as LiquidRuntime,INCREMENTAL_CANDIDATES as LIQUID_CANDIDATES
from app.research.stock_policy import REFERENCES,STOCKS as FOUR
...
def load_inputs():
    files={s:ROOT/'reports/quant_audit_20260912/bars'/f'{s}.parquet' for s in FOUR}
...
def run(output,start='2026-08-31',end='2026-09-11'):
    root=Path(output);root.mkdir(parents=True,exist_ok=False)
    files,frames=load_inputs();dates,audit=audit_bars(frames,start,end)
    ...
    sources=[Path(__file__).resolve(),*[ROOT/'backend/app'/p for p in ('research/incremental_policy.py','research/conditional_policy.py','research/stock_policy.py','research/liquid_policy.py','research/strategy_extensions.py','research/platform.py','research/data_audit.py','research/policy_replay.py','research/causal_week_replay.py','research/attribution.py','research/artifacts.py','quant_policy.py')]]
    r=dict(schema_version=1,status='running',registered_at=pd.Timestamp.now(tz='UTC').isoformat(),evaluation_dates=dates,symbols=list(files),starting_equity=100000,
        ...
        calibration='Past-trained Ridge regression; no calibrated win probabilities',
        traded_symbols=list(FOUR),reference_symbols=list(REFERENCES),excluded_inputs=['MSTR','IBIT'],
        selection_rule=None,
        literature=dict(title='Intraday Patterns in the Cross-section of Stock Returns',authors='Heston, Korajczyk, Sadka',url='https://arxiv.org/abs/1005.3535',transfer='Same-slot historical returns: five-minute single-stock adaptation, not the original broad cross-sectional half-hour replication'),
        hypothesis='Incremental same-slot history, gap interactions, and trailing-range structure separately and together; same five-minute forecast/allocator/cost',
        preregistered_evaluation_panels=[dict(start='2026-08-31',end='2026-09-11'),dict(start='2026-08-24',end='2026-08-28')],
        integrated_risk='One shared three-stock portfolio with prior-session Ledoit-Wolf covariance and turnover costs',
    ...
    for c in LIQUID_CANDIDATES:
        ...
        if c.name=='incremental_control' and start=='2026-08-31' and end=='2026-09-11' and abs(summary['net_pnl']-4702.134021560836)>1e-6:raise AssertionError('Fixed Ridge control mismatch')
        ...
        trial=dict(candidate=asdict(c),cost_bps=5,status='complete',summary=summary,attribution=attribution,
            ...
            subsequent_period=summarize(daily[5:],ledger.marks) if len(daily)>5 else None)
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--output',required=True);p.add_argument('--start',default='2026-08-31');p.add_argument('--end',default='2026-09-11');a=p.parse_args();run(a.output,a.start,a.end)
