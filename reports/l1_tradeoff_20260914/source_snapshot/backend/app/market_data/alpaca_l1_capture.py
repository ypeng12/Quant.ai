...
class AlpacaL1Capture:
    ...
    def _normalize(self, value, event_type: str) -> dict:
    ...
    def _capture(self, value, event_type: str) -> None:
        try:
            self._append(self._normalize(value, event_type))
    ...
    def run(self, api_key: str, secret_key: str) -> None:
        ...
        class ObservedStockDataStream(StockDataStream):
            ...
            async def _start_ws(self):
                collector._transition("connecting")
                try:
                    await super()._start_ws()
                except Exception as error:
                    collector._transition("connection_error", error_type=type(error).__name__)
                    raise
                connection = collector._state["connection_id"] + 1
                collector._transition("connected", authenticated=True, subscribed=False,
            ...
            async def _dispatch(self, message):
                if message.get("T") == "subscription":
                    ...
                    collector._transition("subscribed" if subscribed else "subscription_incomplete",
                    ...
                    collector._transition("server_error", error_code=message.get("code"))
            ...
            async def _consume(self):
                try:
                    ...
                    collector._transition("disconnected", authenticated=False, subscribed=False,
        ...
        try:
            ...
            self._stream = None
            status = "storage_error" if self._fatal_error else "stopped" if self._stop_requested else "stream_ended"
