...
def credentials(env_file=None):
    """Read local credentials without putting secrets in arguments or logs.

    Explicit files take precedence, so a data-only service can use an existing
    private credential file without modifying the trading application's .env.
    """
    explicit = env_file or os.getenv('ALPACA_ENV_FILE')
    if not explicit:
        key = os.getenv('ALPACA_API_KEY')
        secret = os.getenv('ALPACA_SECRET_KEY') or os.getenv('ALPACA_API_SECRET')
        if key and secret:
            return key, secret
    path = Path(explicit).expanduser() if explicit else Path(__file__).resolve().parents[2] / '.env'
    env = dotenv_values(path)
    key = env.get('ALPACA_API_KEY') if explicit else os.getenv('ALPACA_API_KEY') or env.get('ALPACA_API_KEY')
    secret = (env.get('ALPACA_SECRET_KEY') or env.get('ALPACA_API_SECRET')) if explicit else os.getenv('ALPACA_SECRET_KEY') or os.getenv('ALPACA_API_SECRET') or env.get('ALPACA_SECRET_KEY') or env.get('ALPACA_API_SECRET')
