...
class LiquidityRules:
    ...
    def __post_init__(self):
        if not np.isfinite([self.min_price,self.min_adv,self.min_market_cap]).all() or min(self.min_price,self.min_adv,self.min_market_cap)<=0:
...
def daily_eligibility(frames,symbols,day,rules):
    ...
    expected=[str(d.date()) for d in calendar.sessions_in_range(begin,prior_end)[-rules.lookback_sessions:]]
    result={}
    for s in symbols:
    ...
    def prepare(self,c,day,index):
        ...
        if key not in self.daily:
            ...
            for s,row in self.daily[key].items():
                ...
                cap=record.get('market_cap');size_ok=isinstance(cap,(int,float)) and np.isfinite(cap) and cap>=self.rules.min_market_cap
