...
DEFAULT=ROOT/'reports/platform_research_20260913_final'
...
def load_platform_results(directory=DEFAULT):
    ...
    try:
        ...
        for symbol,expected in registry['data_hashes'].items():
            if digest(registry['data_paths'][symbol])!=expected:raise ValueError(f'Data checksum mismatch: {symbol}')
        for name,expected in registry.get('l1_input_hashes',{}).items():
            if digest(name)!=expected:raise ValueError('L1 input checksum mismatch')
