...
def run(args):
    ...
    dates=sorted(set.intersection(*(set(f.index.strftime('%Y-%m-%d')) for f in frames.values())))
    ...
    registry=dict(schema_version=1,registered_at=pd.Timestamp.now(tz='UTC').isoformat(),status='running',
                  ...
                  evaluation_dates=dates,symbols=args.symbols,starting_equity=args.equity,
                  ...
                  limitations=['Same prior-session model fit / next-open fills / intraday flat for all candidates.',
                               ...
                               'Four-stock data is a selected retrospective sample; 30-symbol configurations do not establish historical coverage.',
