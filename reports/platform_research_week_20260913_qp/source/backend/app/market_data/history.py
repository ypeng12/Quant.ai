...
def download_history(symbols, start, end, feed, kind, output, *, max_pages=0):
    ...
    manifest = dict(status='running', source='alpaca_stock_historical', feed=feed, kind=kind,
                    start=start.isoformat(), end=end.isoformat(), symbols=[validate_symbol(s) for s in symbols], files=[])
    ...
    try:
        with requests.Session() as session:
            ...
            for symbol in manifest['symbols']:
                token, pages, records, seen_tokens = None, 0, [], set()
                while True:
                    ...
                    records.extend((payload.get(kind) or {}).get(symbol,[]))
                ...
                if records:
                    if kind=='bars':
                        frame=pd.DataFrame(records).rename(columns={'t':'timestamp','o':'Open','h':'High','l':'Low','c':'Close','v':'Volume'})
                        ...
                        for raw_event in records:
                            ...
                            else: event.update(market_depth='trade_print',price=raw_event['p'],size=raw_event['s'],trade_size_unit='shares',trade_id=raw_event.get('i'),exchange=raw_event.get('x'))
                            ...
                            with path.open('a') as handle: handle.write(json.dumps(event,separators=(',',':'))+'\n')
                save()
                time.sleep(.35)
