#!/usr/bin/env python3
"""Frozen non-crypto per-stock model decisions; no broker imports or order submission."""
import argparse
...
from app.research.stock_policy import StockRuntime as LiquidRuntime,STOCK_CANDIDATES as LIQUID_CANDIDATES
from app.quant_policy import normalize_bars


def register_policy(bundle,directory,candidate='noncrypto_stock_selector'):
    ...
    return register(directory,dict(candidate=c.name,input_symbols=research['symbols'],cost_bps=5,
        ...
        list(dict.fromkeys([*research['sources'],'scripts/shadow_stock_policy.py','backend/app/research/shadow.py','backend/app/research/stock_policy.py'])))
...
def main():
    ...
    q.add_argument('--candidate',default='noncrypto_stock_selector',choices=[c.name for c in LIQUID_CANDIDATES if not(c.control or c.benchmark)])
