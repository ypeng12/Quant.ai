#!/usr/bin/env python3
"""Causal liquid-stock admission and portfolio comparisons, without broker imports."""
...
from app.research.liquid_policy import LiquidRuntime,LiquidityRules,LIQUID_CANDIDATES,LiquidLedger
...
def run(output):
    ...
    runtime=LiquidRuntime(frames,instruments=instruments,research_membership=True)
    sources=[Path(__file__).resolve(),*[ROOT/'backend/app'/p for p in ('research/liquid_policy.py','research/strategy_extensions.py','research/platform.py','research/data_audit.py','research/policy_replay.py','research/causal_week_replay.py','research/attribution.py','research/artifacts.py','quant_policy.py')]]
    r=dict(schema_version=1,status='running',registered_at=pd.Timestamp.now(tz='UTC').isoformat(),evaluation_dates=dates,symbols=list(files),starting_equity=100000,
        ...
        calibration='Nonnegative tree/Ridge blend fit on five earlier walk-forward prediction sessions, sum of coefficients at most one',
        integrated_risk='Prior covariance + causal intraday update + common-reference covariance + validation residual covariance / 5 sessions',
        risk_preferences=dict(risk_aversion=50,gross_limit=.95,reference_risk_multiplier=1,error_risk_multiplier=1,intraday_prior_bars=385),
