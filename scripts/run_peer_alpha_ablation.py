...
def main() -> int:
    ...
    for spec in candidates:
        ...
        stress_runner = ExperimentRunner(frames, sessions, ExecutionConfig(**{**asdict(config), "slippage_bps": 5.0}))
        result = stress_runner.run(dict.fromkeys(EVALUATION_DATES, stressed))
        cost_stress.append({"spec": asdict(stressed), "evaluation_retrospective": result["summary"]})

    output = ROOT / "reports" / "quant_research_20260913" / "peer_alpha_ablation_corrected"
