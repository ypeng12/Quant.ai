#!/usr/bin/env python3
"""Frozen revised direction-model decisions; no broker imports or order submission."""
import argparse
...
from app.research.direction_policy import DirectionRuntime as LiquidRuntime,DIRECTION_CANDIDATES as LIQUID_CANDIDATES
from app.quant_policy import normalize_bars


def register_policy(bundle,directory,candidate='market_curve_ridge'):
    ...
    return register(directory,dict(candidate=c.name,input_symbols=research['symbols'],cost_bps=5,
        ...
        list(dict.fromkeys([*research['sources'],'scripts/shadow_direction_policy.py','backend/app/research/shadow.py','backend/app/research/direction_policy.py'])))
...
def main():
    ...
    q.add_argument('--candidate',default='market_curve_ridge',choices=[c.name for c in LIQUID_CANDIDATES if not(c.control or c.benchmark)])
