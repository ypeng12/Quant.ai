#!/usr/bin/env python3
"""Frozen incremental Alpha model decisions; no broker imports or order submission."""
import argparse
...
def register_policy(bundle,directory,candidate='incremental_all'):
    ...
    return register(directory,dict(candidate=c.name,input_symbols=research['symbols'],cost_bps=5,
        ...
        list(dict.fromkeys([*research['sources'],'scripts/shadow_incremental_policy.py','backend/app/research/shadow.py','backend/app/research/incremental_policy.py'])))
...
def main():
    ...
    q.add_argument('--candidate',default='incremental_all',choices=[c.name for c in LIQUID_CANDIDATES if not(c.control or c.benchmark)])
