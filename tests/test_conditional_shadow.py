...
spec=importlib.util.spec_from_file_location('conditional_shadow',ROOT/'scripts/shadow_conditional_policy.py')
module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)

def test_paper_adapter_records_conditional_horizon_without_assuming_fills(tmp_path,monkeypatch):
    frames=panel();bars=tmp_path/'bars';bars.mkdir()
    for s,f in frames.items():f.to_parquet(bars/f'{s}.parquet')
    manifest=dict(registered_at='2026-08-30T21:00Z',specification=dict(candidate='conditional_state_h6',input_symbols=list(frames),
