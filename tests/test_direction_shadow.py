...
from test_direction_policy import panel,FOUR
from test_liquid_policy import metadata,LiquidityRules
spec=importlib.util.spec_from_file_location('direction_shadow',ROOT/'scripts/shadow_direction_policy.py')
module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)

def test_paper_adapter_records_revised_horizons_and_plan_without_assuming_fills(tmp_path,monkeypatch):
    frames=panel();bars=tmp_path/'bars';bars.mkdir()
    for s,f in frames.items():f.to_parquet(bars/f'{s}.parquet')
    manifest=dict(registered_at='2026-08-30T21:00Z',specification=dict(candidate='market_curve_ridge',input_symbols=list(frames),
    ...
    assert {t['horizon'] for t in r['training']}=={1,6}
    assert all(t['last_train']<'2026-08-31' for t in r['training'])
    assert r['explanation']['planned_steps']==6
    assert r['pnl'] is None and r['orders_submitted']==0
    assert r['explanation']['estimated_terminal_cost']>=0
